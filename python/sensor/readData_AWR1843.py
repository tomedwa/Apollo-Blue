...
def density(x, y):
    ...
    # Compute density around each point
    density = np.zeros(len(X))
    for i in range(len(X)):
        density[i] = np.sum(distances[i] < 3)  # Adjust the radius as needed

    # Find the point with the highest density
    highest_density_point_index = np.argmax(density)
    highest_density_point = X[highest_density_point_index]

    return highest_density_point[0], highest_density_point[1]


def filter(x, y, num, prev):
    zone = [[-0.2, 0.2], [0.1, 0.7]]
    inzone = 0
    for i in range(num):
    ...
    if inzone > 0:
        ...
        x_av, y_av = np.mean(x_nz), np.mean(y_nz)
        x_d, y_d = density(x_nz, y_nz)

        for i in range(num):
            if x[i] == 0:
                x[i] = x_d
                y[i] = y_d
            elif abs(x[i] - x_d) > 0.1:
                x[i] = x_d

        x_d_av = (x_d + x_av) / 2
        y_d_av = (y_d + y_av) / 2

        x_d = check_diff(x_d, prev[0], 0.1)
        x_av = check_diff(x_av, prev[1], 0.1)
        x_d_av = check_diff(x_d_av, prev[2], 0.1)
        y_d = check_diff(y_d, prev[3], 0.2)
        y_av = check_diff(y_av, prev[4], 0.2)
        y_d_av = check_diff(y_d_av, prev[5], 0.2)

        x = np.array([x_d, x_av, x_d_av])
        y = np.array([y_d, y_av, y_d_av])
    else:
